...
uint16_t insert_vert(Geometry *geo, Vertex vert) {
	for (uint16_t i = 0; i < geo->vert_writer; i++)
		if (eq3(geo->vertices[i].pos, vert.pos))
			return i;
...
Geometry *geometry() {
	...
	for (int i = 0; i < LEN(geo->indices); i++) {
		if (i < LEN(geo->vertices))
			geo->vertices[i] = vert_pos(vec3(0.0, 0.0, 0.0));
		geo->indices[i] = 0;
	}
	...
	for (tri = 0; tri < LEN(base_indices); tri += 3) {
		...
		for (x = 0; x <= SIZE; x++) {
			ay = lerp3(a, (float) x / (float) SIZE, c),
			by = lerp3(b, (float) x / (float) SIZE, c);
			rows = SIZE - x;
			for (y = 0; y <= rows; y++)
				if (y == 0 && x == SIZE)
					v[x][y] = ay;
				else 
					v[x][y] = lerp3(ay, (float) y / (float) rows, by);
		}
...
void draw(Mat4 mat, uint16_t art_location[2]) {
